<script lang="ts">
	...
	$: teamRows = (() => {
		const map = new Map<number, { short: string; played: number; wins: number; draws: number; losses: number; pts: number; isHome: boolean }>();
		function get(id: number, short: string) {
			if (!map.has(id)) map.set(id, { short, played: 0, wins: 0, draws: 0, losses: 0, pts: 0, isHome: false });
			return map.get(id)!;
		}
		for (const m of matches) {
			...
			h.played++; h.pts += m.homeScore; h.isHome = true;
			a.played++; a.pts += m.awayScore;
	...
	{#if total > 0}
		...
		<div class="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-8">
			<div class="rounded-lg border border-border bg-muted/30 p-4 text-center">
				<div class="text-xs text-muted-foreground mb-1">Home Win %</div>
				<div class="text-2xl font-bold tabular-nums">{((homeWins / total) * 100).toFixed(1)}%</div>
				<div class="text-xs text-muted-foreground mt-1">{homeWins} of {total} matches</div>
			</div>
			<div class="rounded-lg border border-border bg-muted/30 p-4 text-center">
				<div class="text-xs text-muted-foreground mb-1">Away Win %</div>
				<div class="text-2xl font-bold tabular-nums">{((awayWins / total) * 100).toFixed(1)}%</div>
				<div class="text-xs text-muted-foreground mt-1">{awayWins} of {total} matches</div>
			</div>
			<div class="rounded-lg border border-border bg-muted/30 p-4 text-center">
				<div class="text-xs text-muted-foreground mb-1">Avg Home Score</div>
				<div class="text-2xl font-bold tabular-nums">{avgHomeScore?.toFixed(1) ?? '—'}</div>
			</div>
			<div class="rounded-lg border border-border bg-muted/30 p-4 text-center">
				<div class="text-xs text-muted-foreground mb-1">Avg Away Score</div>
				<div class="text-2xl font-bold tabular-nums">{avgAwayScore?.toFixed(1) ?? '—'}</div>
			</div>
		</div>

		<!-- Per-team breakdown -->
		<h3 class="text-base font-semibold mb-2">Team Points at This Track</h3>
		<div class="mb-8 overflow-x-auto border border-border rounded-lg">
			<SortableTable
				columns={teamColumns}
				rows={teamRows}
